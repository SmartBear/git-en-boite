...
if (!remoteUrl)
  throw new Error("Please define smoke_tests_remote_repo_url env var")
...
describe("smoke test", function () {
  ...
  const branchName = BranchName.of("smoke-tests")
  ...
  async function cleanUpRemoteRepo() {
    if (
      (
        await localRepo.exec("ls-remote", [
          "origin",
          `refs/heads/${branchName}`,
        ])
      ).stdout !== ""
    )
      await localRepo.exec("push", [
        "origin",
        "--delete",
        branchName.toString(),
      ])
  ...
  it("Waits for repo to be fetched", async () => {
    const response = await fetch(
      `${url}/repos/${repoId}/events?until=repo.fetched`
    )
    assertThat(response.status, equalTo(200))
  })
