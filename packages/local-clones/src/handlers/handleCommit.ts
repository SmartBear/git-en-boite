...
export const handleCommit: Handle<GitDirectory, AsyncCommand<Commit>> = async (
  ...
  await repoWithDefaultIndex.withUniqueIndex(async (repo) => {
    ...
    async function addFiles() {
      for (const file of files) {
        ...
        await repo.exec('update-index', ['--add', '--cacheinfo', '100644', objectId, file.path.value])
      }
    }

    async function commitIndex() {
      const treeName = await repo.read('write-tree', [])
      const commitName = await repo.read('commit-tree', [treeName, '-m', message.value, ...commitArgs], {
        env: {
  ...
  async function getParentCommit<ResultType = Promise<void>>(
    ...
    return repoWithDefaultIndex.read('show-ref', ['--hash', commitRef.parent.value]).then(success).catch(failure)
  }
}
